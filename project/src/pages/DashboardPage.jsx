...
//Substituir useEfeect na liha 250
    ...
    try {
      ...
      if (user?.id === "demo-user-id") {
        ...
        const storedIngredients = JSON.parse(
          localStorage.getItem("demo-ingredients") || "[]"
        );
        const storedPackagings = JSON.parse(
          localStorage.getItem("demo-packagings") || "[]"
        );
        ...
        const totalExpenses = storedExpenses.reduce(
          (sum, exp) => sum + Number(exp.monthly_value || exp.unit_cost || 0),
          0
        );
          ...
          const ingredientCost =
            recipe.recipe_ingredients?.reduce(
              (sum, ri) =>
                ...
                Number(ri.quantity) *
                  Number(
                    ri.ingredients.cost_per_unit ||
                      ri.ingredients.costPerUnit ||
                      0
                  ),
              0
            ) || 0;
          const packagingCost = recipe.recipe_packaging?.[0]?.packagings
            ?.unit_cost
            ? Number(recipe.recipe_packaging[0].packagings.unit_cost)
            : 0;
        ...
        const expenseDistribution = storedExpenses
          .slice(0, 5)
          .map((expense) => ({
            name: expense.name,
      ...
      // Load counts
      const [recipesRes, ingredientsRes, packagingsRes, expensesRes] =
        await Promise.all([
      ...
      const totalExpenses =
        expensesRes.data?.reduce(
          (sum, exp) => sum + Number(exp.monthly_value),
          0
        ) || 0;
        ...
        recipesData?.map((recipe) => {
          const ingredientCost =
            recipe.recipe_ingredients?.reduce(
              (sum, ri) =>
                sum +
                Number(ri.quantity) * Number(ri.ingredients.cost_per_unit),
              0
            ) || 0;
          const packagingCost =
